...
use "$temp/ch10_nsd_test", replace

*** Compute Returns ***

foreach file in ch10_nsd_test {
...
use "$temp/ch10_nsd_test.dta", clear
...
twoway (scatter ret var if date == 684, msize(vtiny) msymbol(smx)), legend(label(1 "Pre-Redesign"))

save "$temp\investor_mean_var_`file'.dta", replace	// .0068337
